...
namespace Api.Controllers
{
    ...
    public class TrainController : Controller
    {
        ...
        public JsonResult GetTrain(string trainNumber = "")
        {
            ...
            var html = $"https://www.fernbahn.de/datenbank/suche/?fahrplan_jahr=2023&zug_bereich=&zug_gattung=&zug_nummer={numberRangeIndex}&zug_linie=&ice_typ=&wagengattung=&wagengattung_suche=AND&fv_suche_reihungsverzeichnis=1#fv_suche_reihungsverzeichnis";
            ...
            var correctTrain = doc.DocumentNode.DescendantNodes()
                .Where(x => x.HasClass("zugnr"))
                .FirstOrDefault(x => x.ChildNodes.First().InnerText.Contains(numberParsed.ToString()));

            if (correctTrain is null)
            {
                return Json("Train does not exist");
            }
            ...
            train.FullTrainName = correctTrain.InnerText;

            train.TrainNumber = int.Parse(string.Concat(train.FullTrainName.ToCharArray().Where(char.IsDigit)));
